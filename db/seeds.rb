...
crop_types = ['avacado', 'banana', 'cocoa', 'hempseed', 'palm_oil', 'potato', 'rice', 'soybean']
crop_types.each do |crop|
  found_count = 0
  not_found_count =

  # grab each .csv crop file
  crop_file = "data_files/#{crop}_yield_2007.csv"
  crops = SmarterCSV.process(crop_file)
  # find or create crop
  crop = Crop.find_or_create_by(name: crop)

  # loop through each crop file
  crops.each do |item|
    location = item[:country_or_area]
    value = if item[:value] == '' || item[:value].nil?
              0
            else
              item[:value]
            end
    crop_country = Country.where('name LIKE (?)', "%#{location}%").first

    # dataset contains continents, these entries include a '+' character
    # only country entries should be processed
    unless location.include? '+'
      if crop_country.nil?
        not_found_count += 1
      else
        # create country_crop association between country and crop
        CountryCrop.create(country: crop_country, crop: crop, yield: value)
        found_count += 1
      end
    end
  end
  puts "Matched #{found_count} #{crop.name}s to countries."
  puts "Matches failed for #{not_found_count} #{crop.name}s."
  puts '-' * 50
end